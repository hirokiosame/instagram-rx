const Rx = require('rxjs');
const operators = require('rxjs/operators');
const md5 = require('../utils/md5');
const retry = require('../utils/retry');
const extractSharedData = require('../utils/extractSharedData');
const debug = require('debug');
const _log = debug('IG:User');
const random = (min, max) => (max - min) * Math.random() + min;

class User {

	constructor(request, { username } = {}) {
		this.request = request;
		this.username = username;

		this.initialReq = retry(() => this.request(`/${username}/`))
		.pipe(
			operators.map(src => extractSharedData(src)),
			operators.map(data => [
				data.entry_data.ProfilePage[0].graphql.user,
				data.rhx_gis,
			]),
		);
	}

	details() {
		const log = _log.bind(null, 'details');
		log({ username: this.username });

		return this.initialReq
			.pipe(
				operators.map(([user]) => user),
				operators.tap(undefined, undefined, () => log({ username: this.username }, 'Complete'))
			);
	}

	posts() {
		const log = _log.bind(null, 'posts');
		log({ username: this.username });
		return this.initialReq
			.pipe(
				operators.flatMap(([user, rhx_gis]) => Rx.concat(
					Rx.from(user.edge_owner_to_timeline_media.edges),
					(user.edge_owner_to_timeline_media.edges.length ? this._nextPage(rhx_gis, user.id, user.edge_owner_to_timeline_media.page_info) : []),
				)),
				operators.pluck('node'),
				operators.tap(undefined, undefined, () => log({ username: this.username }, 'Complete'))
			);
	}

	_nextPage(rhx_gis, userId, page_info) {
		const log = _log.bind(null, '_nextPage');

		if (!page_info.has_next_page) { return []; }

		return retry(() => {
			log({ username: this.username, rhx_gis, page_info });

			return this.request.graphql({
				rhx_gis,
				query_hash: 'bd0d6d184eefd4d0ce7036c11ae58ed9',
				variables: {
					id: userId,
					first: 100,
					after: page_info.end_cursor,
				},
			});	
		})
		.pipe(
			operators.delay(random(5000, 15000)),
			// operators.tap(r => console.log({ r })),
			operators.map(({ data }) => data.user.edge_owner_to_timeline_media),
			operators.flatMap(posts => Rx.concat(
				Rx.from(posts.edges),
				this._nextPage(rhx_gis, userId, posts.page_info),
			)),
		);
	}

	following(after = undefined) {
		// Assert logged in

		return this.initialReq.pipe(
			operators.concatMap(([profile, rhx_gis]) => this.request.graphql({
				rhx_gis,
				query_hash: '9335e35a1b280f082a47b98c5aa10fa4',
				variables: {
					id: profile.id,
					first: 24,
					after,
				},
			})),
			operators.map(data => data.data.user.edge_follow),
			operators.flatMap(following => Rx.concat(
				Rx.from(following.edges).pipe(operators.pluck('node')),
				(following.page_info.has_next_page ? this.following(following.page_info.end_cursor) : []),
			)),
		);
	}

	followers(after = undefined) {
		// Assert logged in

		return this.initialReq.pipe(
			operators.concatMap(([profile, rhx_gis]) => this.request.graphql({
				rhx_gis,
				query_hash: '149bef52a3b2af88c0fec37913fe1cbc',
				variables: {
					id: profile.id,
					first: 24,
					after,
				},
			})),
			operators.map(data => data.data.user.edge_followed_by),
			operators.flatMap(followers => Rx.concat(
				Rx.from(followers.edges).pipe(operators.pluck('node')),
				(followers.page_info.has_next_page ? this.followers(followers.page_info.end_cursor) : []),
			)),
		);
	}

	tagged(after = undefined) {
		return this.initialReq.pipe(
			operators.concatMap(([profile, rhx_gis]) => this.request.graphql({
				rhx_gis,
				query_hash: 'e31a871f7301132ceaab56507a66bbb7',
				variables: {
					id: profile.id,
					first: 24,
					after,
				},
			})),
			operators.map(data => data.data.user.edge_user_to_photos_of_you),
			operators.flatMap(tagged => Rx.concat(
				Rx.from(tagged.edges).pipe(operators.pluck('node')),
				(tagged.page_info.has_next_page ? this.tagged(tagged.page_info.end_cursor) : []),
			)),
		);
	}

	stories() {
		return this.initialReq.pipe(
			operators.concatMap(([profile, rhx_gis]) => this.request.graphql({
				rhx_gis,
				query_hash: '45246d3fe16ccc6577e0bd297a5db1ab',
				variables: {
					reel_ids: [profile.id],
					tag_names: [],
					location_ids: [],
					highlight_reel_ids: [],
					precomposed_overlay: false,
				}
			})),
			operators.map(data => data.data.reels_media[0]),
		);
	}


}

module.exports = User;
